...
for (a in alphas) {
  for (b in betas) {
    ...
    dpa_grid[n, 'fitted_base']<-out$par
    dpa_grid[n, 'fitted_ll']<-out$value

    save(dpa_grid, dpa_raw_preds, file='results/dpa_seq.Rdata')
    n=n+1
  }
}
