...
for (a in alphas) {
  for (b in betas) {
    ...
    dpar_grid[n, 'fitted_base']<-out$par
    dpar_grid[n, 'fitted_ll']<-out$value

    save(dpar_grid, dpar_raw_preds, file='results/dpar_seq.Rdata')
    n=n+1
  }
}
