...
dpar_pass_raw_preds<-list()
dpar_pass_grid<-data.frame(
...
for (a in alphas) {
  for (b in betas) {
    dpar_pass_grid[n,'id']<-n
    dpar_pass_grid[n,'alpha']<-a
    dpar_pass_grid[n,'beta']<-b
    
    log<-data.frame(nth=n, alpha=a, beta=b)
    write.csv(log, file='dpar_current')
    ...
    dpar_pass_grid[n,'raw_ll']<-raw_ll
    ...
    dpar_pass_grid[n, 'fitted_base']<-out$par
    dpar_pass_grid[n, 'fitted_ll']<-out$value

    save(dpar_pass_grid, dpar_pass_raw_preds, file='results/dpar_pass_seq.Rdata')
    n=n+1
  }
}
